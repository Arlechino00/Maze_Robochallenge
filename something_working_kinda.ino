/*
  Corridor Following & Turning Robot
  ...
  - If a FRONT wall is detected (both sensors):
    1. Approaches the wall to a set distance.
...
// --- Tuning: P-Control (Turning) - FROM USER'S SKETCH ---
...
void setup() {
  ...
  Serial.println("Corridor/Turner Ready!");
  Serial1.println("Corridor/Turner Ready! Send 'help'.");
  prevTime = millis();
}
...
void handleCorridorFollowing() {
  ...
  if (leftCm < FRONT_WALL_CM && rightCm < FRONT_WALL_CM) {
      ...
      Serial1.println("At wall. Turning 90 deg right.");
      ...
      // 3. Reset PID and yaw
      Serial1.println("Scan complete. Resuming.");
      integral = 0;
      previous_wall_error = 0;
  ...
  // --- Logic Priority 2: Side Wall Pivot (OLD) ---
  ...
  // --- Logic Priority 3: PID Corridor Following (OLD) ---
...
void TurnToAngle(float targetDelta) {
    ...
    Serial1.print("✅ Turn complete. Final Yaw: ");
...
void parseCommand(String command) {
  ...
  // --- General ---
    ...
    Serial1.println("--- Corridor/Turner Help ---");
